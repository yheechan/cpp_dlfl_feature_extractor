...
bool MusicASTVisitor::VisitStmt(clang::Stmt *s)
{
  ...
  // cout << "VisitStmt1" << endl;
  // int numChildren = 0;
  // for (clang::Stmt::const_child_iterator it = s->child_begin(); it != s->child_end(); ++it) {
  //   ++numChildren;
  // }
  // cout<<"number of child: "<<numChildren<<endl;

  if (start_loc.isMacroID() && end_loc.isMacroID())
    return true;
  // cout << "VisitStmt2" << ConvertToString(s, comp_inst_->getLangOpts()) << endl;
  // PrintLocation(src_mgr_, start_loc);
  if (!ValidateSourceRange(start_loc, end_loc))
    return true;
  // cout << "VisitStmt3" << ConvertToString(s, comp_inst_->getLangOpts()) << endl;
  // PrintLocation(src_mgr_, start_loc);
  // PrintLocation(src_mgr_, start_loc);
  // s->dump();
  ...
  if (isa<ReturnStmt>(s))
  {
    ...
    // PrintRange(src_mgr_, *(stmt_context_.decl_stmt_));
    if(stmt_context_.IsInLambda() && stmt_context_.IsInDeclStmt(start_loc) || stmt_context_.IsInConstExpr()){
      // cout<<"meiwenti"<<endl;
      return true;
    }
  }
...
bool MusicASTVisitor::VisitCompoundStmt(clang::CompoundStmt *c)
{
  ...
  // cout << start_loc.printToString(src_mgr_) << endl;
  // cout << end_loc.printToString(src_mgr_) << endl;
  // cout << ConvertToString(c, comp_inst_->getLangOpts()) << endl;
...
bool MusicASTVisitor::VisitExpr(clang::Expr *e)
{
  ...
  if (stmt_context_.IsInRhsOfSpecialAssignmentRange(e)){
      //  cout << "is in special assignment: " <<"expr:"<<ConvertToString(e, comp_inst_->getLangOpts()) << endl;
       return true;
  }

  if (isa<LambdaExpr>(e)){
    // cout<<"found a lambda" << endl;
    // cout << ConvertToString(e, comp_inst_->getLangOpts()) << endl;
    stmt_context_.setIslambdaInDeclstmt(true);
  }
  // cout<<"############"<<endl;
  // cout << ConvertToString(e, comp_inst_->getLangOpts()) << endl;
  // e->dump();
  ...
  if(ExprIsEnumClass(e)){
    // cout<<"321"<<endl;
    stmt_context_.setIsEnumClass(true);
  }
  // cout<<"############"<<endl;
...
bool MusicASTVisitor::VisitEnumDecl(clang::EnumDecl *ed)
{
  ...
  // if(ed->isScoped()){
  //   stmt_context_.setIsEnumClass(true);
  //   cout<<"found enumclass at "<< start_loc.printToString(src_mgr_) << "to " << end_loc.printToString(src_mgr_) << endl;
  // }
  // cout<<"found enumclass and enum at "<< start_loc.printToString(src_mgr_) << "to " << end_loc.printToString(src_mgr_) << endl;
...
bool MusicASTVisitor::VisitTypedefDecl(clang::TypedefDecl *td)
{
  ...
  // cout << ConvertToString(td, comp_inst_->getLangOpts()) << endl;
  // Skip nodes that are not in the to-be-mutated file.
...
bool MusicASTVisitor::VisitVarDecl(clang::VarDecl *vd)
{
  ...
  if(VarTemplateSpecializationDecl* vtsd = dyn_cast<VarTemplateSpecializationDecl>(vd)){
    cout<<"found var template speci"<<endl;
    cout << "start_loc = "; PrintLocation(src_mgr_, start_loc);
    cout << "end_loc = "; PrintLocation(src_mgr_, end_loc);
  }

  if(vd->isConstexpr())
    stmt_context_.setIsInConstExpr(true);
  ...
  if (vd->hasInit())
  {
    ...
    // cout << "Left-hand-side type: "<<vd->getType().getAsString() << endl;
    // bool need_reference_type_rhs_ = false;
  ...
  string need_reference_type_rhs = "reference_wrapper";
  if(vd->getType().getAsString().find(need_reference_type_rhs) != string::npos){
    // cout<< "zhege buxing" << ConvertToString(*init, comp_inst_->getLangOpts()) << endl;
    stmt_context_.setIsInSpecialAssignment(true);
    // cout << "set the special assignment "<< endl;
    stmt_context_.setRhsOfSpecialAssignmentRange(
      new SourceRange(start_loc, end_loc));
  }
  ...
  stmt_context_.setDeclStmtRange(
      new SourceRange(start_loc, end_loc));
...
bool MusicASTVisitor::VisitCXXNewExpr(clang::CXXNewExpr *cne)
{
  SourceLocation start_loc = cne->getBeginLoc();
  SourceLocation end_loc = cne->getEndLoc();
  ...
  if (cne->isArray())
  {
    // cout<<"found a new expression"<<endl;
    if ((!start_loc.isMacroID() && !LocationIsInRange(start_loc, *array_decl_range_)) ||
        (!end_loc.isMacroID() && !LocationIsInRange(end_loc, *array_decl_range_)))
    {  
      stmt_context_.setIsInArrayDeclSize(true);
