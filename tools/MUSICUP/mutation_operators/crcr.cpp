...
bool CRCR::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
	...
	// cout<<"expr: "<<ConvertToString(e, context->comp_inst_->getLangOpts())<<" check: "<<context->IsRangeInMutationRange(SourceRange(start_loc, end_loc))<<endl;
...
void CRCR::Mutate(clang::Expr *e, MusicContext *context)
{
	...
	SourceLocation start_loc = e->getBeginLoc();
	SourceLocation end_loc = GetEndLocOfExpr(e, context->comp_inst_);
	// cout<<"expr: "<<ConvertToString(e, context->comp_inst_->getLangOpts())<<" type check: "<<ExprIsIntegral(context->comp_inst_, e)<<endl;
