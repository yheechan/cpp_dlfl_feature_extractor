...
bool OEAA::IsMutationTarget(clang::Expr *e, MusicContext *context)
{
	...
	if (BinaryOperator *bo = dyn_cast<BinaryOperator>(e))
	{
		...
		// cout<<"###############"<<endl;
		// cout<<"isrange: "<< !context->IsRangeInMutationRange(SourceRange(start_loc, end_loc))<<endl;
		// cout<<"is array: "<< stmt_context.IsInArrayDeclSize()<<endl;
		// cout<<"isenum :"<< stmt_context.IsInEnumDecl()<<endl;
		// cout<<"find: "<< (domain_.find(binary_operator) == domain_.end())<<endl;
		...
		//      - in c++ arithmetic assignment on enum type is not allowed
										 ...
										 CompilerInstance *comp_inst)
{
	...
	if (ExprIsEnum(lhs)||ExprIsEnum(rhs)){
		// cout<<"yinggaiduiya"<<endl;
		return false;
	}
