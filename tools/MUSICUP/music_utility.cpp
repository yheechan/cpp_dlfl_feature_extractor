...
bool ExprIsClass(Expr *e)
{
  return e->getType().getCanonicalType().getTypePtr()->isClassType(); 
}

bool ExprIsEnumClass(Expr *e)
{
  if(!((e->getType()).isNull())){
    if (const clang::EnumType* enumType = e->getType()->getAs<clang::EnumType>()) {
        clang::EnumDecl* enumDecl = enumType->getDecl();
...
bool ExprIsEnum(Expr *e)
{
  // cout<<"type"<<e->getType().getAsString()<<endl;
  if(!((e->getType()).isNull())){ 
    if (const clang::EnumType* enumType = e->getType()->getAs<clang::EnumType>()) {
    // cout<<"ok"<<endl;
        clang::EnumDecl* enumDecl = enumType->getDecl();
...
SourceLocation GetEndLocOfExpr(Expr *e, CompilerInstance *comp_inst)
{
  ...
  if (print)
  {
    cout << "GetEndLocOfExpr: " << ConvertToString(e, comp_inst->getLangOpts()) << endl;
    
    if (DeclRefExpr* de = dyn_cast<DeclRefExpr>(e)){
      cout << "template" <<de->hasTemplateKeyword () <<endl;
    }
  ...
  else if (CallExpr *ce = dyn_cast<CallExpr>(e))
  {
    ...
    if (CXXMemberCallExpr* cxxmce = dyn_cast<CXXMemberCallExpr>(ce)){
      if (print) cout << "cp 13\n";
      // cout<<"start: "<<*(src_mgr.getCharacterData(ret))<<endl;
      // getRParenLoc returns the location before the right parenthesis
      // ret = cxxmce->getRParenLoc();
      while (IsCharAlphabetic(*(src_mgr.getCharacterData(ret))) ||
        IsCharNumeric(*(src_mgr.getCharacterData(ret))) ||
        *(src_mgr.getCharacterData(ret)) == '_' || *(src_mgr.getCharacterData(ret)) == '.'){
          // cout<<"haishi"<<endl;
          ret = ret.getLocWithOffset(1);
          skip_one = true;
          // cout<<"following: "<<*(src_mgr.getCharacterData(ret))<<endl;
      }
      // cout<<"following: "<<*(src_mgr.getCharacterData(ret))<<endl;
      // goto done;
    }

    if (CXXOperatorCallExpr* cxxoce = dyn_cast<CXXOperatorCallExpr>(ce)){
      if (print) cout << "cp 11\n";
      // getRParenLoc returns the location before the right parenthesis
      // ret = cxxmce->getRParenLoc();
      ret = cxxoce->getEndLoc();
      while (IsCharAlphabetic(*(src_mgr.getCharacterData(ret))) ||
        IsCharNumeric(*(src_mgr.getCharacterData(ret))) ||
    ...
    // cout<<"jiazhiqian: "<<*(src_mgr.getCharacterData(ret))<<endl;
    if (!skip_one){ret = ret.getLocWithOffset(1);}
  }
  ...
  else if (MemberExpr *me = dyn_cast<MemberExpr>(e))
  {
    if (print) cout << "cp 4\n";
    ret = me->getMemberLoc();
    // cout<<"startLocation:" <<*(src_mgr.getCharacterData(ret))<<endl;
  ...
  else if (isa<StringLiteral>(e))
  {
    ...
    // cout<<"sourcetype:"<< sourceType.getAsString()<<endl;
    // cout<<"destination type:" << ie->getCastKindName()<<endl;
    ret = ie->getEndLoc();
    if (print) cout<< "cp 9\n";
    if(MemberExpr *mv = dyn_cast<MemberExpr>(subExpr)){
      // cout<<"found class member variable type"<<endl;
      while (IsCharAlphabetic(*(src_mgr.getCharacterData(ret))) ||
        IsCharNumeric(*(src_mgr.getCharacterData(ret))) ||
        *(src_mgr.getCharacterData(ret)) == '_' || *(src_mgr.getCharacterData(ret)) == '.'){
          ret = ret.getLocWithOffset(1);
      ...
      bool skip_one = false;
       while (IsCharAlphabetic(*(src_mgr.getCharacterData(ret))) ||
        IsCharNumeric(*(src_mgr.getCharacterData(ret))) ||
        *(src_mgr.getCharacterData(ret)) == '_' || *(src_mgr.getCharacterData(ret)) == '.'|| *(src_mgr.getCharacterData(ret)) == '\''){
          ret = ret.getLocWithOffset(1);
          skip_one = true;
          // cout<<"following: "<<*(src_mgr.getCharacterData(ret))<<endl;
        }
  ...
  // cout<<GetColumnNumber(src_mgr, ret)<<endl;
  // cout<<"zuihou: "<<*(src_mgr.getCharacterData(ret))<<endl;
  return ret;
}
...
bool IsVarDeclConstexpr(const VarDecl *vd)
{
...
{
  bool check = false;
  if(vd->getType().getAsString().find("array") != string::npos){
    // cout<<"found std::array"<<endl;
    check = true;
  }
  return ((vd->getType()).getTypePtr())->isArrayType() || check;   
}
...
bool IsVarDeclClass(const VarDecl *vd)
{
  return ((vd->getType()).getTypePtr())->isClassType();
}
/**
...
const Stmt* GetParentOfStmt(Stmt *s, CompilerInstance *comp_inst)
{
  ...
  // cout << "find parent size=" << parent_stmt.size() << "\n";
...
const Stmt *GetlambdaParent(Stmt *s, CompilerInstance *comp_inst)
{
  const Stmt* stmt_ptr = s;
...
void ConvertConstIntExprToIntStringAfterTwiddle(Expr *e, CompilerInstance *comp_inst,
                                    string &str, bool option)
{
  Expr::EvalResult result;
  if (e->EvaluateAsInt(result, comp_inst->getASTContext())){
...
void ConvertConstFloatExprToFloatStringAfterTwiddle(Expr *e, CompilerInstance *comp_inst,
                                        string &str, bool option)
{
  llvm::APFloat result(0.0);
  if (e->EvaluateAsFloat(result, comp_inst->getASTContext()))
...
void removeSubString(string& text, string& substring){
...
void StaticCastToType(string &original, string type){
  string prefix = "static_cast<" + type + ">(";
  string postfix = ")";
  original.insert(0, prefix);
  original.append(postfix);
}

void _BoolTobool(string &type_str){
  string from = "_Bool";
  string to = "bool";
  size_t start_pos = 0;
  while ((start_pos = type_str.find(from, start_pos)) != std::string::npos) {
    type_str.replace(start_pos, from.length(), to);
    start_pos += to.length();
  }
}
